...
RATE_LIMIT = 3000  # RPM
CALLS_PER_MINUTE = RATE_LIMIT // 60

@sleep_and_retry
@limits(calls=CALLS_PER_MINUTE, period=1)
def rate_limited_create_embedding(model, text):
...
class TreeBuilder:
    ...
    def create_node(
        ...
        embeddings = {
            model_name: rate_limited_create_embedding(model, text)
    ...
    def create_embedding(self, text) -> List[float]:
        ...
        """
        return rate_limited_create_embedding(
            self.embedding_models[self.cluster_embedding_model], text
        )
